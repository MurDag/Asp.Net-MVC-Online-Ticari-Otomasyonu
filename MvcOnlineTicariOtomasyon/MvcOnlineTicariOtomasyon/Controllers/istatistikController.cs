...
namespace MvcOnlineTicariOtomasyon.Controllers
{
    public class istatistikController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.d15 = deger15;
            var kontrol = c.SatisHarekets.Where(x => x.Tarih == bugun).Count();
            if(kontrol>0)
            { 
            var deger16 = c.SatisHarekets.Where(x => x.Tarih == bugun).Sum(y => y.Toplam).ToString();
            ViewBag.d16 = deger16;
            }
            else
            {
        ...
        public PartialViewResult Partial1()
        {
            var sorgu2 = from x in c.Personels
                         group x by x.Departman_Departmanid.DepartmanAd into g
        ...
        {
            int deger = c.Uruns.Where(x=>x.Durum==true).Count();
